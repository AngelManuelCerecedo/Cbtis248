...
<p style="text-transform: uppercase; font-size: 18px;">
    <strong> <u>C. {{$incidencia->alumno->padre->Nombre}} {{$incidencia->alumno->padre->ApPaterno}} {{$incidencia->alumno->padre->ApMaterno}} </u> <br>
    PADRE DE FAMILIA O TUTOR. <br>
    P R E S E N T E </strong>
</p>
<p style="text-transform: uppercase; text-align: justify; font-size: 18px; font-family: 'Times New Roman', Times, serif;">
    DEL ALUMNO (A) <u>{{$incidencia->alumno->Nombre}} {{$incidencia->alumno->ApPaterno}} {{$incidencia->alumno->ApMaterno}} </u> DEL <u>{{$g}}</u>
    SEMESTRE GRUPO <u>{{$incidencia->alumno->grupo->Clave_Grupo}}</u>
    DE LA CARRERA DE <u>{{$incidencia->alumno->especialidad->Nombre}} </u>, POR ESTE CONDUCTO ME PERMITO CITARLO PARA EL DIA <u> {{$dc}} {{$nd}} DE {{$mc}}</u>
	...
	<tbody>
		<tr>
			...
			<td style="width: 33%; text-align: center;">
				...
				<FONT SIZE=2 class="transformacion2">PSIC. LIZBETH LEAL VENEGAS <br> ORIENTADORA EDUCATIVA</FONT>
			</td>
			<td style="width: 33%; text-align: center;">
			</td>
		</tr>
