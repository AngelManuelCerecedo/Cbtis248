<div>
    @if ($modal)
        @include('livewire.usuarios.modalRU')
    @endif

    @if ($modal2)
        @include('livewire.usuarios.modalEU')
    @endif

    @if ($modal3)
        @include('livewire.usuarios.modalAU', ['user' => $user])
    @endif
    ...
    <div class="container mx-auto px-4 sm:px-8">
        <div class="py-8">
            <div class="pt-8">
                <h1 class="text-4xl font-sans leading-tight text-center">USUARIOS</h1>
            </div>
            <!--DIV PADRE-->
                                ...
                                @foreach ($usuarios as $user)
                                    <tr>
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-base">
                                            <p class="text-gray-900 whitespace-no-wrap">{{ decrypt($user->password2) }}
                                            </p>
                                        </td>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-base">
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-base">
                                            <p class="text-gray-900 whitespace-no-wrap">{{ $user->EstatusUser }}</p>
                                        </td>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <button wire:click="editar({{ $user->id }})" type="button"
                                                class="text-white bg-yellow-400 hover:bg-yellow-500  rounded-lg  text-sm  ml-10 py-2 px-6 m-1"><i
                                                    class="bi bi-pencil-square"></i></button>

                                            <button wire:click="asignarrol({{ $user->id }})" type="button"
                                                class="text-white bg-[#3065AC]  rounded-lg  text-sm  ml-10 py-2 px-6 m-1">
